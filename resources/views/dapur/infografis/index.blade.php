                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="tabelku" class="table table-striped table-bordered table-hover nowrap2" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th width="30%">Judul Infografis</th>
...
{{-- MODAL --}}
<div id="modalku2" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="largeModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/infografis/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'judulinfografis', name: 'judulinfografis'},
                {data: 'gambarsampul', name: 'gambarsampul'},
                {data: 'file', name: 'file'},
            ...
            columnDefs: [
                { className: "ratatengah", "targets": [2,3,4,5,6]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/infografis/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/infografis/edit') }}?id="+id);
	}
