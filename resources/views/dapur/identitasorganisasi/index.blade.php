...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('/dap/identitasorganisasi/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'namaorg', name: 'namaorg'},
    ...
    function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/identitasorganisasi/edit') }}?id="+id);
	}

    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/identitasorganisasi/add') }}");
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/identitasorganisasi/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
                    // processData: false,
