...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('/dap/profilsetjen/getlist') }}",
            columns: [
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/profilsetjen/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/profilsetjen/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/profilsetjen/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
                    // processData: false,
