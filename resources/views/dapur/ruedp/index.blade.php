                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="tabelku" class="table table-striped table-bordered table-hover nowrap2" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th width="15%">Status Penyusunan</th>
                                        <th width="20%">Jumlah Provinsi</th>
                                        <th width="30%">Daftar Provinsi</th>
                                        <th width="10%">Per Tanggal</th>
...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/ruedp/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'statuspenyusunan', name: 'statuspenyusunan'},
                {data: 'jumlah_provinsi', name: 'jumlah_provinsi'},
                {data: 'daftarprovinsi', name: 'daftarprovinsi'},
                {data: 'pertanggal', name: 'pertanggal'},
                {data: 'urutan', name: 'urutan'},
                {data: 'action'}
            ],
            columnDefs: [
                { className: "ratatengah", "targets": [2]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/ruedp/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/ruedp/edit') }}?id="+id);
	}
