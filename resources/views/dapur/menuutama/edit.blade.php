<style>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group mb-1">
                    ...
                    <input type="input" class="form-control form-control form-control-sm" value="{{ $data->nama_menu }}" id="nama_menu" name="nama_menu" style="width: 100%;">
                    <input type="hidden" class="form-control form-control form-control-sm" value="{{ $data->id_menuutama }}" id="id_menuutama" name="id_menuutama" style="width: 100%;">
                </div>
                ...
                <div class="form-group mb-1">
                    ...
                    <select class="form-control form-control-sm" id="menu_induk" name="menu_induk" style="width: 60%">
                        @if ($menuinduk->count() == 0)
                            <option>Mohon input menu utama publikasi terlebih dahulu</option>
                        @else
                            @foreach ($menuinduk->get() as $kat)
                                <option value="{{ $kat->id_menuutama }}" {{ $kat->id_menuutama === $data->id_parent ? "selected" : "" }}>
                                    {{ $kat->nama_menu }}
                                </option>
                            @endforeach
                        @endif
    ...
    $.validator.setDefaults({
      submitHandler: function(){
        ...
        $.ajax({
            url: "{{ url('/dap/menuutama/saveupdate') }}",
            type: 'POST',
    ...
    $(document).ready(function(e){
        $('#nama_menu').focus();
        if ($('#is_parent').is(":checked")){
            $('#menu_utama').prop('disabled', true)
        }else{
            $('#menu_utama').prop('disabled', false)
        }
