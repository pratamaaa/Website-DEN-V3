                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="tabelku" class="table table-striped table-bordered table-hover nowrap2" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th width="15%">Ringkasan Isi</th>
                                        <th width="15%">Satker</th>
                                        <th width="15%">Penanggungjawab</th>
                                        <th width="10%">Waktu/Tempat</th>
                                        <th width="10%">Bentuk Informasi</th>
                                        <th width="10%">Retensi Arsip</th>
                                        <th width="10%">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/dip/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'ringkasanisi', name: 'ringkasanisi'},
                {data: 'picsatker', name: 'picsatker'},
                {data: 'pic', name: 'pic'},
                {data: 'waktutempat', name: 'waktutempat'},
                {data: 'bentukinformasi', name: 'bentukinformasi'},
                {data: 'retensiarsip', name: 'retensiarsip'},
                {data: 'action'}
            ],
            columnDefs: [
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/dip/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/dip/edit') }}?id="+id);
	}
