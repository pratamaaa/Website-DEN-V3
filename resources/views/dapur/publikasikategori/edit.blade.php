<style>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group mb-1">
                    ...
                    <input type="input" class="form-control form-control form-control-sm" value="{{ $data->nama_kategori }}" id="nama_kategori" name="nama_kategori" style="width: 100%;">
                    <input type="hidden" class="form-control form-control form-control-sm" value="{{ $data->id_publikasi_kategori }}" id="nama_kategori" name="id_publikasi_kategori" style="width: 100%;">
                </div>
                <div class="form-group mb-1 mt-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_parent" name="is_parent" {{ $data->id_parent == "0" ? "checked" : "" }}> 
                    ...
                    <select class="form-control form-control-sm" id="kategori_induk" name="kategori_induk" style="width: 80%">
                        ...
                        @else
                            @foreach ($kategoriinduk->get() as $kat)
                                <option value="{{ $kat->id_publikasi_kategori }}" {{ $kat->id_publikasi_kategori === $data->id_parent ? "selected" : "" }}>
                                    {{ $kat->nama_kategori }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <select class="form-control form-control-sm" id="group_infopublik" name="group_infopublik" style="width: 60%">
                        ...
                        @else
                            @foreach ($groupinfopublik->get() as $grp)
                                <option value="{{ $grp->id_informasipublik }}" {{ $grp->id_informasipublik === $data->id_informasipublik ? "selected" : "" }}>
                                    {{ $grp->informasipublik }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div class="form-group mb-1">
                    ...
                    <input type="input" class="form-control form-control form-control-sm" value="{{ $data->url }}" id="url" name="url" style="width: 100%;">
                </div>
                <div class="form-group mb-1">
    ...
    $.validator.setDefaults({
      submitHandler: function(){
        ...
        $.ajax({
            url: "{{ url('/kategoripublikasi/saveupdate') }}",
            type: 'POST',
