...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('kategoripublikasi/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'namakategori', name: 'namakategori'},
                {data: 'kategoriinduk', name: 'kategoriinduk'},
            ...
            columnDefs: [
                { className: "ratatengah", "targets": [3,6]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/kategoripublikasi/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/kategoripublikasi/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/kategoripublikasi/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
