...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/organisasiden/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'katjabatan', name: 'katjabatan'},
            ...
            columnDefs: [
                { className: "ratatengah", "targets": [4,5,6,7]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/organisasiden/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/organisasiden/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/organisasiden/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
