                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="tabelku" class="table table-striped table-bordered table-hover nowrap2" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th width="20%">Nama Grouping</th>
                                        <th width="5%">Jenis</th>
                                        <th width="10%">Kelompok</th>
                                        <th width="30%">Alamat Url</th>
...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('dap/infopublik-grouping/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'informasipublik', name: 'informasipublik'},
                {data: 'tipe', name: 'tipe'},
            ...
            columnDefs: [
                { className: "ratatengah", "targets": [2,3,5,6,7]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/infopublik-grouping/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/infopublik-grouping/edit') }}?id="+id);
	}
    ...
    function hapus(id){
		...
		swal({
			...
			if (willDelete) {
				$.ajax({
					url: "{{ url('/dap/infopublik-grouping/delete') }}",
					type: 'POST',
					data: {"id":id},
					dataType: 'json',
