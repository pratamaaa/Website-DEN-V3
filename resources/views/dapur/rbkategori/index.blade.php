                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="tabelku" class="table table-striped table-bordered table-hover nowrap2" width="100%">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th width="30%">Kategori RB</th>
                                        <th width="25%">Kategori RB (En)</th>
                                        <th width="20%">Slug</th>
...
<script type="text/javascript">
    ...
    $(function (){
        var table = $('#tabelku').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('/dap/kategorirb/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'namarb', name: 'namarb'},
                {data: 'namarben', name: 'namarben'},
                {data: 'slugrb', name: 'slugrb'},
                {data: 'urutanrb', name: 'urutanrb'},
                {data: 'action'}
            ],
            columnDefs: [
                { className: "ratatengah", "targets": [4,5]},
    ...
    function showFormadd(){
    	$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategorirb/add') }}");
	}
	function showFormedit(id){
		$('#modalku').modal('show').find('#modalku_content').load("{{ url('/dap/kategorirb/edit') }}?id="+id);
	}
